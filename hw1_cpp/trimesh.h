...
public:
    ...
    // new code for IFT 6113
    std::vector< int > get_face_from_he_index(index_t he_index) {
        std::vector< int > face;
        face.resize(3);
        const halfedge_t& he = m_halfedges[ he_index ];
        face[0] = he.to_vertex;
        face[1] = m_halfedges[he.next_he].to_vertex;
        face[2] = m_halfedges[m_halfedges[he.next_he].next_he].to_vertex;
        return face;
    };

    Eigen::MatrixXi get_faces(){
        Eigen::MatrixXi Faces(m_face_halfedges.size(),3);
        for (index_t i=0; i<Faces.rows(); ++i){
            index_t he_index = m_face_halfedges[i];
            std::vector< int > face = get_face_from_he_index(he_index);
            Faces.row(i) = Eigen::Vector3i(face.data());
//            Faces.row(i) = face;
        }
        return Faces;
    };
